		...
		$('#dg').datagrid({
			...
			onClickCell : function(index, field, value) {
				if (rowindex != undefined) {
					//获取数量
					var edBodAmount = $('#dg').datagrid('getEditor', {
						index : rowindex,
						field : 'bodAmount'
					});
					...
					//获取进价
					var edBodBuyPrice = $('#dg').datagrid('getEditor', {
						index : rowindex,
						field : 'bodBuyPrice'
					});
...
<body>
	...
	<form class="myfm" style="padding:10px;margin:0px;background:#eee;"
		id="ff" action="">
		<div class="myftitle">请填写采购信息</div>
		<div class="myfitem2">
			<label>供应商：</label><input id="supName" class="easyui-searchbox"
				name="supName" /> <input type="hidden" id="supId" name="supId" />
			<label for="shId">仓库类型：</label><select id="cc"
				class="easyui-combobox" name="shId">
				<c:forEach items="${applicationScope.sysParam.shId}" var="sh">
					<option value="${sh.key}">${sh.key}--${sh.value}</option>
				</c:forEach>
			</select> <label>日期：</label><input type="text" id="boDate"
				class="easyui-datebox" name="boDate" required="required" />
		</div>
		<div class="myfitem2">
			<label> 应付：</label><input type="text" id="boPayable" name="boPayable" />
			<label>已付：</label><input type="text" id="boPaid" name="boPaid" /> <label>欠款：</label><input
				type="text" id="boArrears" name="boArrears" />
		</div>
		<div class="myfitem2">
			<label>原始单号：</label><input type="text" id="boOriginal"
				name="boOriginal" /> <label>经手人：</label><input type="text"
				id="boAttn" name="boAttn" /> <label>操作员：</label><input type="text"
				id="boOperator" name="boOperator" />
		</div>
		<div class="myfitem2">
